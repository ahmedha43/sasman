...
يقدم هذا الدليل شرحاً شاملاً وتفصيلياً لكافة عناصر واجهة إدارة أجهزة الـ **NAS (Network Access Server)** في نظام **SASMAN**، وكيفية إعداد وربط راوترات **MikroTik** سواء عبر بروتوكول **UDP التقليدي السريع** أو عبر بروتوكول **RadSec (TLS / mTLS)** المشفر والمخصص للفروع البعيدة التي تعمل خلف **CGNAT**.
...
5. [آلية التحكم والقطع العكسي خلف CGNAT (Reverse Disconnect)](#5-آلية-التحكم-والقطع-العكسي-خلف-cgnat-reverse-disconnect)
6. [جدول المقارنة بين UDP و RadSec TLS](#6-جدول-المقارنة-بين-udp-و-radsec-tls)
7. [استكشاف الأخطاء والتشخيص (Troubleshooting)](#7-استكشاف-الأخطاء-والتشخيص-troubleshooting)
...
1. ⚡ **`إعداد راديوس سريع (نظام ومايكروتك)`**:
...
```mermaid
sequenceDiagram
    participant MT as راوتر المايكروتك البعيد (خلف CGNAT)
    participant SAS as سيرفر SASMAN المركزي (بورت 2083)

    Note over MT,SAS: 1. المصافحة المتبادلة (Mutual TLS Handshake)
    MT->>SAS: إرسال شهادة العميل (agent.crt) والاتصال عبر TCP :2083
    SAS->>MT: التحقق من الشهادة والتوثيق المتبادل (mTLS)
    Note over MT,SAS: 2. إنشاء قناة اتصال دائمة ومستقرة
    MT->>SAS: طلب مصادقة المشترك (Access-Request) مشفرة
    SAS-->>MT: رد الموافقة والسرعات (Access-Accept) مشفرة
    Note over SAS,MT: 3. القطع العكسي (Reverse Disconnect)
    SAS->>MT: إرسال أمر طرد المشترك المنتهي عبر نفس قناة الـ TLS
    MT-->>SAS: تأكيد الطرد (Disconnect-ACK)
```
...
## 5. آلية التحكم والقطع العكسي خلف CGNAT (Reverse Disconnect)
...
## 6. جدول المقارنة بين UDP و RadSec TLS
...
## 7. استكشاف الأخطاء والتشخيص (Troubleshooting)
