...
func NewReader(r io.Reader) Reader {
	// Use a 64 KB buffer to comfortably handle large RouterOS API responses
	// that would otherwise exceed the default 4 096-byte bufio buffer and
	// trigger slice-bounds panics inside the proto layer.
	return &reader{
		ctxReader: &ctxReader{Reader: bufio.NewReaderSize(r, 65536), done: make(chan struct{})},
	}
}
